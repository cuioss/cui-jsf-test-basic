---
name: release
description: Cut a cui-jsf-test-basic release — bump .github/project.yml version, open and merge the release PR, wait for the automated Release workflow, verify the release landed, then reformat the generated GitHub release notes
user-invocable: true
...
Cuts a new cui-jsf-test-basic release end-to-end: determine the version, open the version-bump PR
...
the tagging, Maven Central deploy and GitHub release creation.
...
- `release.current-version` (currently `4.3.0`) — the **last released** version.
- `release.next-version` (currently `4.4-SNAPSHOT`) — what `pom.xml` carries between releases.

**The choice is genuinely open here** and the skill must not guess:
`current-version` is `4.3.0` but the pom floats on `4.4-SNAPSHOT`, so `current-version` does
**not** sit on the `next-version` line.

- **`4.4`** — strip `-SNAPSHOT`, starting the new minor line; `next-version` then
  moves to the following minor.
- **`4.3.1`** — continue the patch line; `next-version` stays `4.4-SNAPSHOT`.
...
```bash
gh pr list --repo cuioss/cui-jsf-test-basic --state open --json number,title,isDraft
```
- **No open PRs** → proceed.
...
The Maven CI workflow only triggers on `main, "feature/*", "fix/*", "chore/*", "release/*"`. Any other prefix skips the `build`
check and blocks the merge:

```bash
git checkout -b chore/release_4.4
```
...
- `next-version:` → for a new minor line, the following minor + `-SNAPSHOT`; for a patch
  release, **leave unchanged** (`4.4-SNAPSHOT`)
...
if ! gh label list --repo cuioss/cui-jsf-test-basic --search skip-bot-review --json name --jq '.[].name' \
     | grep -qx skip-bot-review; then
  gh label create skip-bot-review --repo cuioss/cui-jsf-test-basic \
    --description "Skip automated bot review" --color ededed
fi
gh pr create --repo cuioss/cui-jsf-test-basic --base main \
...
```bash
gh pr checks <pr#> --repo cuioss/cui-jsf-test-basic --watch
```
...
- Every review comment MUST get a reply and MUST be resolved — fix it and say so, or explain
  ...
  ```bash
  gh api --paginate repos/cuioss/cui-jsf-test-basic/pulls/<pr#>/comments
  ```
- **Unresolved review threads block the merge.** A PR can show every check green and still
  ...
  ```bash
  gh api graphql --paginate -f query='query($endCursor:String){repository(owner:"cuioss",name:"cui-jsf-test-basic"){pullRequest(number:<pr#>){reviewThreads(first:100,after:$endCursor){pageInfo{hasNextPage endCursor} nodes{id isResolved}}}}}'
...
```bash
gh pr merge <pr#> --repo cuioss/cui-jsf-test-basic --squash --delete-branch
```
...
```bash
gh run list --repo cuioss/cui-jsf-test-basic --workflow "Release" --limit 3 \
  --json status,conclusion,displayTitle,databaseId
gh run watch <databaseId> --repo cuioss/cui-jsf-test-basic
```

### Step 11 — Verify the release landed

```bash
gh release view <version> --repo cuioss/cui-jsf-test-basic --json tagName,name,isDraft,createdAt
git fetch --tags && git tag --list <version>
curl -sfI https://repo1.maven.org/maven2/de/cuioss/test/cui-jsf-test-basic/<version>/cui-jsf-test-basic-<version>.pom \
...
```bash
mkdir -p .plan/temp
gh release view <version> --repo cuioss/cui-jsf-test-basic --json body --jq .body \
  > .plan/temp/release-<version>-orig.md
# ...build the reformatted body in .plan/temp/release-<version>.md...
gh release edit <version> --repo cuioss/cui-jsf-test-basic --notes-file .plan/temp/release-<version>.md
```
...
collapsed or dropped, and a reminder that consumer-bump PRs will appear in `cuioss-parent-pom`.
