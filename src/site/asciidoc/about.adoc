...
Sometimes it is helpful to test, whether a navigation has occurred.
If your test-class implements `JsfEnvironmentConsumer` it implicitly provides assertions for navigation: 
...
class FacesTestEnabledTest implements JsfEnvironmentConsumer {
    ...
    @Test
    void handleNavigationWithoutBack() {
        // Some preconditions
        // ...
        
        // Either check, whether a navigation with an outcome took place 
        assertNavigatedWithOutcome(HomePage.OUTCOME);
        
        // Or assert whether a certain redirect url was called
        assertRedirect("my/view.xhtml");
    }
